...
### Not yet built (deferred to Stage 2/3 — by design)
The other two review surfaces — **`classification.csv`** (machine-proposed type per item) and **`person-map.csv`** (harvested people → dedup/confirm) — are **generated by the scripts** (`wp si:classify` and the person-harvest pass), not hand-seeded. Creating empty templates now would risk drifting out of sync with the script output. **Build them when you write those scripts in Stage 2/3**, so the columns match exactly what the tooling emits. Their column schemas already live in `03-classification-ruleset.md` (classification.csv) and `04-youtube-ingestion-spec.md` / `02-migration-outline.md` (person-map.csv) for reference.
