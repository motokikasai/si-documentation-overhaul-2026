/* Schiller Institute homepage draft — hero v3 (scroll-driven WebGL).
 *
 * Changes vs. hero-v2.js (v2 kept intact):
 *  - The planet's night lights brighten (~15%) as the network completes:
 *    development literally lights the world.
 *  - City flare on connection: each node flares the moment the drawing
 *    corridor tip reaches it — cities visibly join the network.
 *  - Corridor maturation: feeder lines start pale and warm to full trunk
 *    gold once the pulses start flowing (planned → operational).
 *
 * Inherited from v2 (vs. hero.js v1):
...
function init(renderer) {
  ...
  const glowVert = /* glsl */ `
    ...
    void main() {
      ...
      vAlpha = aAlpha * uOpacity * tw * (1.0 + 0.35 * flare);
      vec4 mv = modelViewMatrix * vec4(position, 1.0);
      gl_PointSize = aSize * (280.0 / -mv.z) * (0.9 + 0.1 * tw) * (1.0 + 0.22 * flare);
  ...
  function glowPoints({ positions, sizes, colors, alphas, phases, twinkle, reaches }) {
  ...
  const FEEDER_COLD = new THREE.Color(0xb5a486);
  const TRUNK_GOLD = new THREE.Color(0xe8c87e);
  ...
  const corridorLines = CORRIDORS.map(({ chain, tier }, ci) => {
    ...
    const mat = new THREE.LineBasicMaterial({
      color: tier === 0 ? TRUNK_GOLD.clone() : FEEDER_COLD.clone(),
      transparent: true,
      opacity: baseOpacity,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
  ...
  function renderFrame() {
    ...
    earthUniforms.uGrowth.value = ap;
    corridorLines.forEach(({ line, total, delay, baseOpacity, tier }) => {
      ...
      if (tier === 1) {
        ...
        line.material.color.lerpColors(FEEDER_COLD, TRUNK_GOLD, pulseOn);
        line.material.opacity = (baseOpacity + 0.2 * pulseOn) * fade;
      } else {
