#!/usr/bin/env python3
"""Render the JASPER draft's poster from the Jasper scene.

A copy of wp-plugins/si-hero-earth/build/make-poster.py pointed at this
draft. It exists because the poster is a frame of the scene, and this draft's
scene has a different palette — using the shipped poster here would show the
old field for a second and then dissolve into the new one, which is exactly
the fault we spent 0.2.0 and 0.2.1 removing.

    python3 articles/build/serve.py 8761          # from projects/schiller-wp-rebuild/
    python3 homepage-draft/hero-jasper/build/make-poster.py

Serve from the repo's prototype root, not from this folder: the draft borrows
the plugin's textures and Jasper's fonts by relative path.
...
OUT = ROOT / "assets" / "img"
URL = "http://localhost:8761/homepage-draft/hero-jasper/"
