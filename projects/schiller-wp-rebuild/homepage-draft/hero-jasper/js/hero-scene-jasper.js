/* si/hero-earth — the scene, JASPER.
 * ===========================================================================
 * A duplicate of wp-plugins/si-hero-earth/assets/js/si-hero-scene.js. The
 * choreography, the shaders, the corridor data and the handoff are unchanged
 * — DIFF IT, that is the point. Only the palette below is different, and it
 * is the same set of tokens css/hero-jasper.css uses, because the canvas,
 ...
 *   css/hero-jasper.css. The two must stay equal, and the poster must be
 ...
 *
 * ---------------------------------------------------------------------------
 * PORTED to WordPress as the si/hero-earth block (si-hero-earth plugin).
 * The choreography below is the prototype's, unchanged. Only the seams moved:
 *   - three is imported from the tree-shaken vendor bundle, by relative path,
 *     so no import map is needed and the module resolves wherever WP puts it.
 *   - the six DOM handles are scoped to one block instance and passed in as
 *     `ctx` instead of being global querySelectors, so two heroes can coexist.
 *   - texture URLs come from the block's config, not hardcoded relative paths.
 *   - initHero() returns false instead of setting a .no-webgl class: the
 *     static hero is what the server already rendered, so failure means
 *     'do nothing', not 'swap something in'.
 * See README.md 'Porting notes' for the full diff rationale.
 */

import * as THREE from "../../../wp-plugins/si-hero-earth/assets/vendor/three-slim.js";
...
/**
 * Upgrade one server-rendered static hero into the scroll-driven WebGL scene.
 * @param {HTMLElement} root  the .si-hero element
 * @param {object} cfg        { tex: {day,night,clouds,moon,nightHi|null} }
 * @returns {boolean}         false if WebGL is unavailable (caller leaves the
 *                            static hero exactly as it is)
 */
export function initHero(root, cfg) {
  const ctx = {
    heroEl: root,
    canvas: root.querySelector(".si-hero__canvas"),
    stages: Array.from(root.querySelectorAll(".si-hero__stage")),
    scrollHint: root.querySelector(".si-hero__scrollhint"),
    chapterBtns: Array.from(root.querySelectorAll(".si-hero__chapters button")),
    cfg,
  };
  if (!ctx.canvas || !ctx.stages.length) return false;
...
function init(renderer, ctx) {
  ...
  function updateStages(p) {
    stages.forEach((el, i) => {
      ...
      /* PORT CHANGE (a11y): a fully faded-out act is still in the document,
       * so without this the email field in act 3 is reachable by Tab and
       * readable by a screen reader while it is invisible on screen, and
       * all four headlines are announced at once. Only fully-invisible
       * acts are removed — mid-crossfade both are legitimately on screen. */
      const gone = op < 0.005;
      if (el.inert !== gone) el.inert = gone;
  ...
  if (window.ResizeObserver) new ResizeObserver(resize).observe(ctx.canvas);
