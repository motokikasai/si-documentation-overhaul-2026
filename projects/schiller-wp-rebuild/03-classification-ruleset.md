...
| R4.2 | **Conference** (`si_conference`) from a **post** | `post_type = post` AND (≥2 **embedded** YouTube videos — anchors excluded) AND (a `Panel n`/`Session n` heading OR an agenda heading OR ≥3 printed `Name, Affiliation` lines OR an event title) — OR the post is already named in `conference-map.csv` / embeds a video `video-segmentation.csv` files under a `conference_key` | Medium | review | **Added 2026-09-20.** R4 tested `post_type = page` OR portfolio and R8 (the `post` default) excludes only R3/R5/R5.1/R5.2 — so a conference published as a blog post could match nothing and fell to R9 default-keep. Measured over all 4,140 published posts by `tools/day3-conference-posts.py`: 650 posts carry event evidence, 207 need a decision, 155 of them would otherwise migrate as plain Articles. Contract: `01-csv-contracts.md` §5b |
