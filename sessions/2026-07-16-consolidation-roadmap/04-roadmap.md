...
|---|---|---|---|---|
| **P0-a. Pre-access work** (NEW — can start NOW) | REST census refresh; Screaming Frog full crawl (license it); yt-dump of all playlists/videos/captions; classification-rule prototyping on `posts.csv`+REST; ratify 10-topic set with leadership | 10–14 | — | none |
| **P0-b. Access + clone** | Access bundle (WP admin · IONOS panel · SSH/SFTP · DB) from host owner; clone prod → Local; PHP 8.3 locally; WPML baseline counts; integrity check (homoglyphs, `schiller-custom-scripts`) | 8–12 | — | **G1 access meeting** |
| **P1. Census + model** | DB census (portfolio taxonomy!, shortcode census); register taxonomies + 7 CPTs (revised Pods mu-plugin); hand-made sample of each type incl. relationships; seed terms | 14–20 | — | — |
| **P2. Classification & persons** | `si:classify` (R-rules minus Forecast; R5.1 by category) → classification.csv; `si:persons` → person-map.csv; term-map finalization from `category-map-draft.csv` | 26–38 | 30–50 (CSV review waves) | **G7 team availability** |
...
| **P4. Media + YouTube pipeline** | `si:media` (Folders + Document promotion); pipeline v2 stages A–G → Conferences + Presentations (5-case) + transcript batch | 34–52 | 15–25 (playlist + segmentation CSVs) | — |
...
| **P6. Staging replay** | Fresh live clone → IONOS `dev.` (PHP 8.3, noindex, basic-auth); replay ALL scripts + approved CSVs unattended; delta-classify the tail; team QA round 2 | 8–14 | 8–12 | staging capability confirmed |
...
| **P8. Hardening + cutover** | Caching (LiteSpeed/WPSC per host audit) + Cloudflare + WebP batch; UpdraftPlus + hardening; SEO plugin (sitemap/OG/JSON-LD/hreflang); GDPR carry-over + nocookie embeds; freeze → `si:delta` → redirects live → DNS flip → **GSC DNS-verify + submit sitemap** → monitor 14 days | 14–22 | 6–10 (test transactions, monitoring) | avoid conference weeks |
| **Total** | | **216–324 h** | **87–144 h** | |
